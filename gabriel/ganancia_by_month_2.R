...
kexperimento  <- 1054   #NA si se corre la primera vez, un valor concreto si es para continuar procesando

kscript         <- "ganancia_by_month_stack"

karch_dataset    <- "./datasets/dataset_partitioned_stack.csv"   #este dataset se genero en el script 812_dataset_epic.r

#cargo los datos donde voy a aplicar el modelo
dapply  <- fread("./datasets/dataset_partitioned_stack_kaggle.csv")


kBO_iter    <-  150   #cantidad de iteraciones de la Optimizacion Bayesiana
ktrain_subsampling  <- 1.0

#Aqui se cargan los hiperparametros
hs <- makeParamSet( 
  makeNumericParam("learning_rate",    lower=    0.01 , upper=    0.15),
  makeNumericParam("feature_fraction", lower=    0.1  , upper=    1.0),
  makeIntegerParam("min_data_in_leaf", lower=  2500L   , upper= 4000L),
  makeIntegerParam("num_leaves",       lower=    300L   , upper= 600L),
  #makeIntegerParam("max_bin",       lower=    31L   , upper= 1023L),
  makeNumericParam("min_gain_to_split", lower=   0.0 , upper= 0.7),
...
ksemilla_azar  <- 372225  #Aqui poner la propia semilla
